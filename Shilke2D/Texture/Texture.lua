...
function Texture:getColor(x,y)
	local r,g,b,a = self:getRGBA(x,y)
...
function Texture:getRegionPx(resultRect)
	--it's equivalent to getRect for a Texture
	local res = resultRect or Rect()
	res.x = 0
	res.y = 0
	res.w = self.width
...
--helpers for hitTest call
local __helperRect = Rect()
local __helperRect2 = Rect()
...
function Texture:hitTest(p1,a1,texture2,p2,a2)
	assert(self.srcData and texture2.srcData,  "Texture:hisTest called Texture generated by MAOIFrameBufferTexture") 
    return imageHitTestEx(self.srcData,p1,a1,
        texture2.srcData,p2,a2,
		self:getRegionPx(__helperRect),
		texture2:getRegionPx(__helperRect2),
		self.rotated,
		texture2.rotated)
end
